    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content bg-dark text-white">
        <div class="modal-header d-flex justify-content-between align-items-center border-secondary-subtle">
          <div>
            <h4 class="fw-bold mb-0 text-white">{{ mode === 'edit' ? 'Edit Chapter' : 'Create Chapter' }}</h4>
        ...
        <div class="modal-body">
          <form>
            <div class="mb-3">
              <label class="form-label text-white">Chapter Name</label>
              <input type="text" class="form-control bg-dark text-white border-secondary" placeholder="Enter chapter name" v-model="form.name" />
            </div>
            <div class="mb-3">
              <label class="form-label text-white">Description</label>
              <textarea rows="4" class="form-control bg-dark text-white border-secondary" placeholder="Enter chapter description..." v-model="form.description"></textarea>
            </div>
          </form>
        </div>
...
const props = defineProps({
  ...
  chapterData: { type: Object, default: () => ({ name: '', description: '', id: null }) }
})

const form = ref({ name: '', description: '', id: null })

watch(() => props.chapterData, (val) => {
  form.value = { ...val }
}, { immediate: true })
